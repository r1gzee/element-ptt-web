...
> {
     ...
     * Defaults to always returning true when not provided.
     * @param header - The header to check
     * @returns true if the header can be focused
     */
    isHeaderFocusable?: (header: Header) => boolean;
...
export function GroupedVirtualizedList<Header, Item, Context>(
    ...
    const wrappedIsEntryFocusable = useCallback(
        (entry: NavigationEntry<Header, Item>): boolean =>
            "header" in entry ? (isHeaderFocusable?.(entry.header) ?? true) : isItemFocusable(entry.item),
        [isHeaderFocusable, isItemFocusable],
    );
    ...
    const getItemComponentInternal = useCallback(
        (index: number, _groupIndex: number, _item: unknown, context: VirtualizedListContext<Context>): JSX.Element =>
            getItemComponent(index, context, onFocusForItem),
